...
/**
 * Build View | Edit cell HTML (baked provider mark; no runtime JS).
...
function buildEditActionsHtml (opts) {
  ...
  parts.push(
    `<a class="adt-edit-inline-link" href="${escapeHtml(href)}" ` +
      ...
      `${icon}<span class="adt-edit-inline-text">${escapeHtml(editLabel)}</span>` +
      `</a>`
  )
...
function buildEditActionsAsideHtml (opts) {
  ...
  if (!inner) return ''
  return `<aside class="page-context page-context-actions" role="note">${inner}</aside>`
}

/**
 * Insert View|Edit as a lead-table Source row, or fall back to a floating aside.
 * @param {string} html
...
function injectEditActions (html, actionsHtml) {
  ...
  // Prefer appending inside an existing lead table body.
  const leadTableBody =
  ...
  // Lead aside without a table (byline-only): add a one-row table after inner content.
  const leadOpen = /(<aside\b[^>]*\bpage-context-lead\b[^>]*>)([\s\S]*?)(<\/aside>)/i
  if (leadOpen.test(html)) {
    const table = `<table class="page-context-table"><tbody>${sourceRow}</tbody></table>`
    return html.replace(leadOpen, (_, open, inner, close) => `${open}${inner}${table}${close}`)
  }

  // No lead: standalone actions aside (legacy placement).
  return `<aside class="page-context page-context-actions" role="note">${actionsHtml}</aside>${html}`
}
